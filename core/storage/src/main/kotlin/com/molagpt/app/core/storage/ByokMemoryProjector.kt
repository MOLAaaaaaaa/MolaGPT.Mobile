...
object ByokMemoryProjector {
    ...
    fun usageRules(memoryTools: Boolean, recallTool: Boolean): String {
        ...
        return buildString {
            append("## 记忆\n\n")
            if (memoryTools) {
                append("用户透露了跨对话仍然成立的信息时，用 save_memory 记一条。")
                append("判断标准：换一个新对话，不知道这件事会不会让你的回答变差。\n\n")
                append("不要记：本轮的临时上下文、你推断而用户没确认的结论、随口提到的话题、")
                append("能从对话记录直接查到的事实。\n\n")
                append("绝不要记：密钥、密码、令牌、身份证件、支付信息；")
                append("以及种族、民族、宗教信仰、性取向、性生活、政治观点、犯罪记录、健康与病史。\n\n")
                append("记的时候用完整的第三人称陈述句，不要用「这个」「刚才」这类指回本轮的词。")
                append("source_quote 必须逐字取自用户本轮说的话。\n\n")
